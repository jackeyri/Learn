...
int Index(SqString s, SqString t) {  // 简单匹配算法
    ...
    while (i < s.length && j < t.length) {
        if (s.data[i] == t.data[j]) {  // 继续区配下一个字符
            ++i;
            ++j;
    ...
    if (j >= t.length)
        return i - t.length;  // 返回匹配的的一个字符的下标
    else
        return -1;  // 模式匹配不成功
}
void GetNext(SqString t, int next[]) {  // 由模式串t求取next值
    int j, k;
    j = 0;
    k = -1;
    next[0] = -1;
...
void GetNextval(SqString t, int nextval[]) {  // 有模式串t求出nextval值
    int j = 0, k = -1;
    nextval[0] = -1;
    while (j < t.length) {
